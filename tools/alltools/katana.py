...
#     flags = []
#     # silent: blank or "y" → yes, "n" → no
#     sil = data.get("silent", "").strip().lower()
#     if sil == "" or sil == "y":
#         flags.append("-silent")

#     # ─── js? [y/n] (default: y)
#     js = data.get("js", "").strip().lower()
#     if js == "" or js == "y":
#         flags.append("-jc")

#     # ─── headless? [y/n] (default: n)
#     hl = data.get("headless", "").strip().lower()
#     if hl == "y" or hl =="":
#         flags.append("-hl")

#     # ─── concurrency (-c) (default: 5)
#     conc = data.get("concurrency", "").strip() or "5"
#     flags += ["-c", conc]
...
#     # only show the filename in the UI
...
#             # katana often prints parse/help messages to stdout, not stderr
#             err = result.stderr.strip() or result.stdout.strip() or "Unknown katana error"
...
#         traceback.print_exc()
#         return {"status": "error", "message": f"Katana exception: {str(e)}"}
