...
(() => {
  ...
  const state = {
    ...
    nodes: new Map(),    // id -> {id, x, y, width, height, el, handles}
    edges: new Map(),    // id -> {id, fromId, toId, pathEl}
    needsRender: true,
    draggingNode: null,  // { id, pointerId, startMouseWorld:{x,y}, startNode:{x,y} }
    panning: null,       // { pointerId, startScreen:{x,y}, startCam:{x,y} }
  ...
  function createNodeFromTool(toolMeta, worldX, worldY) {
    ...
    // Cache metrics once (width/height & handle offsets relative to node)
    // Measure after append: safe single layout read now, none during drag
    ...
    nodeEl.querySelector('.node-action-btn.delete')?.addEventListener('click', () => deleteNode(id));
  ...
  function finishConnect(toNodeId) {
    ...
    state.edges.set(id, { id, fromId: state.connecting.fromId, toId: toNodeId, pathEl });
  ...
  function onCanvasPointerDown(e) {
    // Middle button OR right button OR hold spacebar (detected via class)
  ...
  function onWheelZoom(e) {
    // ctrl+wheel or normal wheel both zoom the canvas
    e.preventDefault();
  ...
  // Space bar toggles panning cursor (optional UX sugar)
  ...
  function render() {
    if (state.needsRender) {
      ...
      // Edges (and rubber band if connecting)
      ...
      if (state.connecting?.rubber) {
        // Rubber from fromNode output to current mouse position
        ...
        if (from) {
          ...
          // read last mouse position from a cached variable (updated on pointermove)
  ...
  el.canvas.addEventListener('click', (e) => {
    if (e.target === el.canvas || e.target === el.content) cancelConnect();
  });

  // ---------- Library drag→create ----------
  // We’ll use native HTML5 DnD to keep it simple.
  ...
  function boot() {
    setupLibrary();
    requestAnimationFrame(render);
    log('Canvas ready. Tip: Space + drag to pan. Mouse wheel to zoom. Right/middle button also pans.');
  }
