...
async function boot() {
  ...
  // ignore aborts (route changes) and only toast real errors
  window.addEventListener("unhandledrejection", (ev) => {
    const r = ev.reason;
    const msg = String(r?.message || r || "").toLowerCase();
    const name = String(r?.name || "").toLowerCase();
    if (name === "aborterror" || msg.includes("abort")) return; // ← important
