...
// // Fit canvas to parent width and device pixel ratio; return {ctx,width,height} in CSS px
// function prepCanvas(canvas, cssHeight = 260) {
//   const dpr = window.devicePixelRatio || 1;
//   const parent = canvas.parentElement;
//   const cssWidth = Math.max(300, (parent?.clientWidth || canvas.clientWidth || canvas.width || 900));
...
// // ---------- legacy simple charts (still available) ----------
// export function drawLineChart(canvas, data) {
//   const { ctx, width, height } = prepCanvas(canvas, 320);
//   ctx.clearRect(0, 0, width, height);
...
//   const arr = Array.isArray(data) ? data : [];
//   if (arr.length === 0) { drawGrid(ctx, width, height, padding, 5, 5); return; }
...
// // internal base renderer (no event bindings) so hover can reuse it
// function renderTimeSeriesBase(ctx, width, height, points, opts) {
//   const paddingLeft = 50, paddingRight = 16, paddingTop = 16, paddingBottom = 34;
//   ctx.clearRect(0, 0, width, height);
...
//     ctx.fill();
//   }
//   return { xScale, yScale, paddingTop, paddingBottom };
// }
...
//   // base
//   const scales = renderTimeSeriesBase(ctx, width, height, points, opts);
...
//     let bestI = 0, bestDX = Infinity;
//     for (let i=0; i<points.length; i++){
//       const dx = Math.abs(scales.xScale(+points[i].x) - mx);
//       if (dx < bestDX){ bestDX = dx; bestI = i; }
//     }
//     const p = points[bestI];
...
//     renderTimeSeriesBase(ctx, width, height, points, opts);
//     ctx.strokeStyle = 'rgba(255,255,255,0.25)';
//     ctx.lineWidth = 1;
//     ctx.beginPath(); ctx.moveTo(x, scales.paddingTop); ctx.lineTo(x, height - scales.paddingBottom); ctx.stroke();
...
//     tip.innerHTML = `<b>${df}</b><br>${formatK(p.y)} scans`;
//   };
//   canvas.onmouseleave = () => { tip.style.display='none'; renderTimeSeriesBase(ctx, width, height, points, opts); };
// }
...
function chooseYAxis(max, { integer = false, ticks = 5 } = {}) {
  if (integer) {
    const step = Math.max(1, Math.ceil(max / ticks));           // step is an integer
    const axisMax = Math.max(step * ticks, 1);                   // axis max is integer
    return { step, axisMax };
  }
...
function renderTimeSeriesBase(ctx, width, height, points, opts) {
  ...
  const xMin = Math.min(...xs), xMax = Math.max(...xs);
  const yMax = Math.max(1, Math.max(...ys));
  const { step, axisMax: yAxisMax } = chooseYAxis(yMax, { integer: opts.integer, ticks: 5 });
  ...
  for (let y = 0; y <= yAxisMax + 1e-6; y += step) {
    ...
    ctx.font = '12px system-ui, sans-serif';
    ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
    ctx.fillText(opts.integer ? String(Math.round(y)) : formatK(y), paddingLeft - 8, yy);
  }
...
export function drawTimeSeriesChart(canvas, points, opts = {}) {
  ...
  canvas.onmousemove = (evt) => {
    ...
    const df = (opts.bucket === 'month')
      ? d.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })
      : d.toLocaleDateString(undefined, { weekday:'short', month:'short', day:'numeric' });
    tip.innerHTML = `<b>${df}</b><br>${opts.integer ? Math.round(p.y) : p.y} scans`;
  };
  canvas.onmouseleave = () => { tip.style.display='none'; renderTimeSeriesBase(ctx, width, height, points, opts); };
}

export function drawBarChartLabeled(canvas, labels, values, opts = {}) {
  const { ctx, width, height } = prepCanvas(canvas, 320);
  ctx.clearRect(0,0,width,height);
  const padding = 50, bottom = 40, right = 16, top = 16;
  const plotW = width - padding - right, plotH = height - top - bottom;
  ...
  const n = Math.max(1, values.length);
  const maxVal = Math.max(1, ...values);
  const { step, axisMax: yAxisMax } = chooseYAxis(maxVal, { integer: opts.integer, ticks: 5 });
  ...
  for (let y=0; y<=yAxisMax+1e-6; y+=step){
    ...
    ctx.beginPath(); ctx.moveTo(padding, yy); ctx.lineTo(width - right, yy); ctx.stroke();
    ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.textAlign='right'; ctx.textBaseline='middle'; ctx.font='12px system-ui';
    ctx.fillText(opts.integer ? String(Math.round(y)) : formatK(y), padding - 8, yy);
  }
  ...
  canvas.onmousemove = (e)=>{
    ...
    tip.style.left = (hit.x + hit.w/2) + 'px';
    tip.style.top  = (hit.y) + 'px';
    tip.innerHTML = `<b>${hit.label || 'Tool'}</b><br>${opts.integer ? Math.round(hit.value) : hit.value} uses`;
  };
  canvas.onmouseleave = () => tip.style.display='none';
}

// export prepCanvas for other modules if needed
export { prepCanvas };
