...
def ensure_daily_grant(user_id: int, grant_mic: int = DAILY_FREE_CREDITS, ref_prefix: str = "daily_") -> None:
    ...
    db.session.add(LedgerEntry(
        user_id=user_id, type=LedgerType.GRANT, bucket=CreditBucket.DAILY,
        amount_mic=grant_mic, ref=f"{ref_prefix}{today_utc.isoformat()}"
    ))
...
def debit(user_id: int, cost_mic: int, ref: Optional[str] = None) -> Dict[str, int]:
    # idempotent retry: if a prior DEBIT with same ref exists, return its breakdown
    if ref:
